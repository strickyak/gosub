...
func (tv *ForwardTV) Type() TypeValue   { return tv.Resolve().Type() }
...
func (co *Compiler) VisitCall(x *CallX) Value {
	...
	if len(funcRec.Outs) != 1 {
		...
		return &SimpleValue{c, &MultiTV{multi}}
	} else {
		c := Format("(%s)(%s)", funcVal.ToC(), strings.Join(argc, ", "))
		t := funcRec.Outs[0].TV
